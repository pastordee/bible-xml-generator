...
def strip_glued(xml, heading, report):
    """Remove `heading` where it is welded onto preceding text inside a <v>.

    The import concatenated the heading with no separator, so the giveaway is a
    non-whitespace character immediately before it and only markup/whitespace
    after it. Anything else is left alone.
    """
    pattern = re.compile(
        r'(?<=\S)' + re.escape(heading) + r'(?=\s*(?:<crossref\b|<woc\b|</woc>|</v>|$))')
