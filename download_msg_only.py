#!/usr/bin/env python3
"""Download only MSG (The Message) Bible version."""
...
def main():
    print("="*60)
    print(f"DOWNLOADING {VERSION} (The Message)")
    print("="*60)
    
    # Count existing files
    existing = count_existing_files(VERSION)
    total = sum(book['chapters'] for book in BOOKS)
    print(f"\n📊 Current Status: {existing}/{total} chapters ({existing*100//total if total else 0}%)")
