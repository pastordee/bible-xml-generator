#!/usr/bin/env python3
"""
Smart resumable download script that tracks progress per version
...
PROGRESS_FILE = "api_bible_download_progress.json"
API_KEY = "pyExkJPN1XXpoJ39Xa8Xi"

# Complete Bible book information (all 66 books with chapter counts)
BOOKS = [
    # Old Testament
...
VERSIONS = ["KJV", "WEB", "ASV", "BSB", "CEV", "FBV", "GNV", "DRA", "BRS", "LSV", "NKJV", "AMP", "NLT"]
...
def load_progress():
    ...
    if os.path.exists(PROGRESS_FILE):
        with open(PROGRESS_FILE, 'r') as f:
            return json.load(f)
    return {"versions_completed": [], "current_version": None, "chapters_downloaded": 0}
...
def count_existing_files(version):
    """Count how many files already exist for a version."""
    version_dir = version.lower()
    if not os.path.exists(version_dir):
        return 0
    return len([f for f in os.listdir(version_dir) if f.endswith('.xml')])

def download_version_smart(version, delay=2):
    """Download a specific version, skipping existing files."""
    version_dir = version.lower()
    ...
    if not bible_metadata:
        print(f"  ⚠️  Could not fetch metadata for {version}")
        return False
    ...
    for book in BOOKS:
        for chapter_num in range(1, book['chapters'] + 1):
            ...
            # Download chapter
            print(f"  📥 {book['title']} {chapter_num}...", end='', flush=True)
...
def main():
    print("="*60)
    print("SMART RESUMABLE API.BIBLE DOWNLOADER")
    print("="*60)
    ...
    for version in VERSIONS:
        count = count_existing_files(version)
        status = "✅ Complete" if count >= 1189 else f"🔄 {count}/1189 ({count*100//1189}%)"
    ...
    for version in VERSIONS:
        ...
        if version in progress["versions_completed"]:
            print(f"\n⏭️  Skipping {version} (already completed)")
            continue
        
        # Check if mostly complete
        existing = count_existing_files(version)
        if existing >= 1189:
            print(f"\n✅ {version} already complete ({existing} files)")
            progress["versions_completed"].append(version)
        ...
        success = download_version_smart(version)
        
        if success:
            progress["versions_completed"].append(version)
            progress["chapters_downloaded"] += existing
            save_progress(progress)
            print(f"\n✅ {version} complete!")
        else:
            print(f"\n⚠️  {version} stopped (may need to resume)")
