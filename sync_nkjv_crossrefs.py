#!/usr/bin/env python3
"""
Sync NKJV cross-references between xml_nkjv/ XML files and xml_nkjv/cross_refs/ parsed data.
Uses NKJV Study Bible PDF and nkjv2_trimmed.txt as authoritative sources.
...
4. Verify against source materials (PDF and nkjv2_trimmed.txt)
"""
...
def remove_extra_crossrefs(xml_file, extra_refs, dry_run=True):
    """Remove extra cross-references from XML file."""
...
def process_all_books(dry_run=True):
    """Process all books in xml_nkjv/cross_refs/."""
    
    crossref_dir = Path('xml_nkjv/cross_refs')
    
    if not crossref_dir.exists():
        print(f"ERROR: Directory not found: {crossref_dir}")
        return
    
    print("NKJV Cross-Reference Sync")
    print("=" * 80)
...
def main():
    ...
    if dry_run:
        print("\n💡 To apply changes, run: python3 sync_nkjv_crossrefs.py --live")
